...
const PurchaseInvoiceVoucher = () => {
  ...
  function calculateSum(type) {
    ...
    if (type === "CreditSum") {
      if (invoiceAmount?.length > 0) {
        creditsum += parseFloat(invoiceAmount);
        return creditsum;
      } else {
        return creditsum;
      }
    } else return debitsum;
  }
  ...
  const customButton = (
    <FormGroupButton
      title="Add Purchase Invoice Voucher"
  ...
  return (
    <FinanceCrudComponent
      formName="Purchase Invoice Voucher"
...
export default PurchaseInvoiceVoucher;
